    ...
    table: {
      ...
      body: [
        ...
        [
          celdaDato(
            'Área tratada:',
            cert.areaTratada != null && cert.areaTratada !== ''
              ? `${cert.areaTratada} m²`
              : ''
          ),
          celdaDato(
...
function bloqueFirmas(firmaDirector, firmaGerente) {
  function columna(imagen, titulo) {
    const stack = [];
    if (imagen) {
  ...
  return {
    columns: [
      columna(firmaDirector, 'Director técnico'),
...
function construirDocDefinition(cert, assets = {}) {
  const { logoBase64, firmaDirector, firmaGerente } = assets;
